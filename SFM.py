...
class PoseEstimator:
    ...
    def recover_pose_and_triangulate(self, E, pts1, pts2):
        ...
        for R in possible_rotations:
            for T in possible_translations:
                ...
                pts3d = cv2.triangulatePoints(self.K @ np.hstack((np.eye(3), np.zeros((3, 1)))), P, pts1.T, pts2.T)
                pts3d /= pts3d[3, :]
        ...
        if len(self.projections) == 1:
            self.correspondences[1] = {tuple(pts2[i]): tuple(best_points[:, i]) for i in range(pts2.shape[0])}
    ...
    def incremental_pose_estimation(self, descriptors, keypoints):
        ...
        for i in range(1, len(descriptors) - 1):
            pts1, pts2 = self.find_correspondences(keypoints[i], keypoints[i+1], self.correspondences[i])
            _, rvec, tvec, _ = cv2.solvePnPRansac(np.array(pts1), np.array(pts2), self.K, None)
            ...
            self.correspondences[i+1] = {tuple(pts2[j]): tuple(pts3d[:3, j]) for j in range(pts2.shape[0])}
...
class StructureFromMotion:
    ...
    def visualize_results(self):
        ...
        def plot_camera_poses(poses, title):
            ...
            layout = go.Layout(title=title, scene=dict(xaxis=dict(title='X Axis'), yaxis=dict(title='Y Axis'), zaxis=dict(title='Z Axis')))
            fig = go.Figure(data=traces, layout=layout)
            fig.show()

        if self.estimator.trajectories:
            plot_point_cloud(np.hstack(self.estimator.trajectories), 'Initial 3D Reconstruction')
